...
rule compare_IC50s:
    """compare predicted IC50s between mAbs."""
    input:
        [
        ...
        nb="notebooks/compare_IC50s.ipynb",
    output:
        nb="results/notebooks/compare_IC50s.ipynb",
    log:
        os.path.join(config["logdir"], "compare_IC50s.txt"),
    conda:
        "dms-vep-pipeline/environment.yml"
    shell:
